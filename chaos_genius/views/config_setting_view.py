...
def get_onboarding_status():
    ...
    try:
        data_sources = (
            DataSource.query.filter(DataSource.active == True).first()
            is not None
        )
        kpis = Kpi.query.filter(Kpi.active == True).first() is not None
        if kpis:
            kpi_list = Kpi.query.filter(
                Kpi.active == True, Kpi.anomaly_params != None
            ).all()

            analytics = len(kpi_list) > 0

        organisation_settings_value = ConfigSetting.query.filter(
            ConfigSetting.name == "organisation_settings", ConfigSetting.active == True
        ).all()
        organisation_settings = len(organisation_settings_value) > 0
